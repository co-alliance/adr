...
<xsl:stylesheet xmlns:mods="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
	...
	<!-- Branched from LOC Revision 1.14 by Colorado Alliance of Research Libraries for Colorado State Publications Library.-->
	...
	<xsl:template name="marcRecord">
		...
		<xsl:for-each select="marc:datafield[@tag='245']">
			<mods:titleInfo>   
				...
				<!-- 2009-05-19 Added mapping to display the intitial articles in the mods:title. Applied "greater than or equal to" processing script. -->
				<!-- 2010-04-01 Incorporated subtitle from 245 |b into main title, separated by colon. -->
				<xsl:if test="@ind2&gt;=0">
					<mods:title>
						<xsl:value-of select="$titleChop"/>
						<xsl:call-template name="chopPunctuation">
							<xsl:with-param name="chopString">
								<xsl:call-template name="subfieldColon">    
									<xsl:with-param name="axis">b</xsl:with-param>
...
<xsl:for-each select="marc:datafield[@tag='246']">
			<!-- 2012-04-16 All ind2 values need to map to Alternative Title, commenting this out: <xsl:if test="@ind2=' ' or @ind2=0">-->
		...
		<!-- 2010-04-06 Commenting this out because uniform title below was missing |a.  
...
<xsl:for-each select="marc:datafield[@tag='130']|marc:datafield[@tag='240']">
		...
		<xsl:for-each select="marc:datafield[@tag='100']">
			<mods:name type="personal">
				<xsl:call-template name="nameABCQ"/>    <!-- 2010-04-02 Do not transform 100|d. -->
				<xsl:call-template name="affiliation"/>
				<mods:role>
					<mods:roleTerm authority="marcrelator" type="text">creator</mods:roleTerm>
				</mods:role>
				<!-- 2012-04-06 Added hardcoded role name for Islandora form. Commented out: <xsl:call-template name="role"/>-->
			</mods:name>
		</xsl:for-each>
		...
		<xsl:for-each select="marc:datafield[@tag='700'][not(marc:subfield[@code='t'])]">
			<mods:name type="personal">
				<xsl:call-template name="nameABCQ"/>   <!-- 2010-04-02 Do not transform 700|d. -->
				<xsl:call-template name="affiliation"/>
				<mods:role>
		...
		<!-- 2009-11-16 Put 710 all on one line -->
		...
		<xsl:for-each select="marc:datafield[@tag=260]/marc:subfield[@code='a']">
				<xsl:if test="not(contains(.,'S.l.'))"> <!-- 2010-04-07 Do not populate Place of Publication if 260|a has S.l. -->
				<mods:place>
					<mods:placeTerm>
						...
						<xsl:call-template name="chopPunctuationFront">
							<xsl:with-param name="chopString">
								<xsl:call-template name="chopPunctuation">
									...
									<xsl:with-param name="chopString" select= "translate(.,'[]','')" />   <!-- 2010-04-06 Remove all brackets -->
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
					</mods:placeTerm>
			...
			<xsl:for-each
				...
				<xsl:choose>
					<xsl:when test="@code='b' and not(contains(.,'s.n.'))" > <!-- 2010-04-07 Do not populate Publisher if 260|b has s.n. -->
						<mods:publisher>
							<xsl:call-template name="chopPunctuation">
								<xsl:with-param name="chopString" select="translate(.,'[]','')"/>   <!-- 2010-04-06 Remove all brackets -->
			...
			<xsl:for-each select="marc:leader">
				<!-- Not currently using this field in Islandora 6
				<mods:issuance>  
					<xsl:choose>
	...
	<!--2012-02-13 Populate "form" fields even though they won't display in Islandora. -->	
...
<xsl:for-each select="marc:datafield[@tag=504]">
		...
		<xsl:for-each
			...
			@tag=562 or @tag=565 or @tag=567 or @tag=580 or @tag=581 or @tag=583 or @tag=584 or @tag=585 or @tag=586 or @tag=588]">
			<mods:note>
				<xsl:call-template name="uri"/>
				<xsl:variable name="str">
		...
		<!--  2009-06-04  Added 098 as <mods:classification> -->
		...
		<xsl:for-each select="marc:datafield[@tag=776]">
			<!-- 2009-11-19 Do not populate 776 Related Title
		...
		<mods:recordInfo>
			...
			<xsl:for-each select="marc:controlfield[@tag=001]">
				<mods:recordIdentifier>
					...
					<xsl:value-of select="substring(substring-after(.,'oc'),2)"/> <!-- 2010-04-20 Select all after ocn or ocm -->
					</mods:recordIdentifier>
			</xsl:for-each>
	...
	<xsl:template name="subjectGeographicZ">
		<!-- 2010-04-05 Deduplication of geographic subjects-->
	...
	<xsl:template name="subjectTemporalY">
		<!-- 2010-05-14 Deduplication (modified) of temporal subjects -->
	...
	<xsl:template name="subjectTopic">
		<!-- 2010-05-14 Deduplication (modified) of topic subjects -->
		<xsl:variable name="teo">
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString" select="."/>
			</xsl:call-template>
	...
	<xsl:template name="subjectGenre">
		<!-- 2010-05-14 Deduplication (modified) of subject genre -->
	...
	<xsl:template name="nameABCDN">
		<xsl:for-each select="marc:subfield[@code='a']">
			<mods:namePart>
				...
				<xsl:text>, </xsl:text>
				<!-- 2009-11-25 Add period at the end of |a -->
			</mods:namePart>
		</xsl:for-each>
		<xsl:for-each select="marc:subfield[@code='b']">
			<mods:namePart>
	...
	<xsl:template name="nameABCDN_CSPL">
		<!-- 2009-11-13 Display these on a single line: 110, 710, 100, 700 -->
		<xsl:for-each select="marc:subfield[@code='a']">
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString" select="."/>
				<xsl:with-param name="punctuation">:,;/</xsl:with-param>  
	...
	<xsl:template name="nameABCQ">     <!-- 2010-04-02 same as ABCDQ but without "d"/nameDate -->
		<mods:namePart>
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString">
					<xsl:call-template name="subfieldSelect">
	...
	<xsl:template name="subfieldColon">  <!-- 2010-04-01 Inserts a colon in front of subtitle. -->
		<xsl:param name="anyCodes"/>
		<xsl:param name="axis"/>
		<xsl:param name="beforeCodes"/>
		<xsl:param name="afterCodes"/>
		<xsl:variable name="str">
			<xsl:for-each select="marc:subfield">
				<xsl:if
					test="contains($anyCodes, @code)      or (contains($beforeCodes,@code) and following-sibling::marc:subfield[@code=$axis])      or (contains($afterCodes,@code) and preceding-sibling::marc:subfield[@code=$axis])">
					<xsl:text> : </xsl:text>     <!-- colon with spaces on either side -->
					<xsl:value-of select="text()"/>
					<xsl:text> </xsl:text>
				</xsl:if>
			</xsl:for-each>
			...
			<mods:name type="corporate">
				<xsl:for-each select="marc:subfield[@code='a']">
					<mods:namePart>
						...
						<xsl:value-of select="../marc:subfield[@code='t']"/> <!-- 2010-04-10 Attach 610|t immediately after -->
					</mods:namePart>
				</xsl:for-each>
				<xsl:for-each select="marc:subfield[@code='b']">
					<mods:namePart>
	...
	<xsl:template match="marc:datafield[@tag=651]">
		<mods:subject>
			...
			<xsl:for-each select="marc:subfield[@code='a']">
				...
				<!-- 2010-04-15 Deduplication of geographic values-->
				<xsl:variable name="geo">
					<xsl:call-template name="chopPunctuation">
						<xsl:with-param name="chopString" select="."/>
					</xsl:call-template>
				...
				<xsl:variable name="before"> <!-- unchopped string of all preceding nodes with tag "6xx" and code z -->
					...
					<xsl:for-each select="../preceding-sibling::*[@tag='651']/marc:subfield[@code='a']"> 
						<xsl:value-of select="."/>
					</xsl:for-each>
				</xsl:variable>
				<xsl:if test="not(contains($before, $geo))">
